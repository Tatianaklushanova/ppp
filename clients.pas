...
///// извлечение результатов из файла при сбое///////////
...
begin
   ...
   if MyClient.Strings[2] ='' then  DBLookupComboBox5.KeyValue:= Null
    ...
    DBRadioGroup1.ItemIndex:= strtoint(MyClient.Strings[4]);  // пол
    TDBEdit1.Text:= MyClient.Strings[5];        // фамилия
    TDBEdit2.Text:= MyClient.Strings[6];        // имя
    TDBEdit3.Text:= MyClient.Strings[7];        // отчество
    DateTimePicker1.DateTime := StrToDateTime(MyClient.Strings[8]);    // дата рождения
    DBLookupComboBox1.KeyValue:= strtoint(MyClient.Strings[9]);      // профессия
    DBLookupComboBox1.datasource.dataset.fieldbyname('Profesion').AsInteger:=strtoint(MyClient.Strings[9]);

    DBLookupComboBox2.KeyValue:= strtoint(MyClient.Strings[10]);  // предприятие
    ...
    if MyClient.Strings[11] ='' then  begin
       ...
       DBLookupComboBox3.datasource.dataset.fieldbyname('Cex').AsInteger:=0;
        ...
        DBLookupComboBox3.KeyValue:= strtoint(MyClient.Strings[11]);  // цех
...
begin
  MyClient:= TStringlist.create;
    ...
    MyClient.Add(inttostr(DBRadioGroup1.ItemIndex));  // пол
    MyClient.Add(TDBEdit1.Text);        // фамили
    MyClient.Add(TDBEdit2.Text);        // имя
    MyClient.Add(TDBEdit3.Text);        // отчество
    MyClient.Add (DateTimeToStr(DateTimePicker1.DateTime));    // дата рождения
    MyClient.Add(inttostr(DBLookupComboBox1.KeyValue));  //профессия
    MyClient.Add(inttostr(DBLookupComboBox2.KeyValue));  //предприятие
    MyClient.Add(vartostr(DBLookupComboBox3.KeyValue));  // цех
...
{$ENDIF}
 ...
 if not FileExists('client.txt') then ZapFileClient; // запоминание в файл client
